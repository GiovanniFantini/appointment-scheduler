...
function MerchantSubscriptionEditor({ merchantId }: { merchantId: number }) {
  ...
  return <section className="plan-editor">
    ...
    {error && <p className="error-banner" role="alert">{error} <button data-activity="admin.pages.MerchantSubscriptionTab.1" onClick={() => { setError(''); void load() }}>Riprova</button></p>}
    ...
    {assignment ? <>
      ...
      <form data-activity="admin.pages.MerchantSubscriptionTab.2" onSubmit={e => { e.preventDefault(); void perform('assign') }}><fieldset disabled={saving}>
        <div className="plan-fields"><label>Pacchetto<select data-activity="admin.pages.MerchantSubscriptionTab.3" required value={planId} onChange={e => setPlanId(e.target.value)}><option value="">Seleziona un pacchetto</option>{plans.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
          <label>Tipo di assegnazione<select data-activity="admin.pages.MerchantSubscriptionTab.4" value={trial ? 'trial' : 'active'} onChange={e => setTrial(e.target.value === 'trial')}><option value="active">Definitiva, senza scadenza</option><option value="trial">Prova gratuita</option></select></label></div>
        ...
        {(trial || assignment.trialEndsAt) && <label className="plan-days">Giorni di prova / proroga<input data-activity="admin.pages.MerchantSubscriptionTab.5" type="number" min={1} max={3650} step={1} required value={days} onChange={e => setDays(Number(e.target.value))} /></label>}
        ...
        <div className="plan-actions"><button data-activity="admin.pages.MerchantSubscriptionTab.6" className="btn-primary" disabled={!selected} type="submit">{saving ? 'Salvataggio…' : 'Assegna pacchetto'}</button>
          {assignment.trialEndsAt && <><button data-activity="admin.pages.MerchantSubscriptionTab.7" className="btn-secondary" type="button" disabled={!Number.isInteger(days) || days < 1 || days > 3650} onClick={() => void perform('extend')}>Proroga prova</button><button data-activity="admin.pages.MerchantSubscriptionTab.8" className="btn-secondary" type="button" onClick={() => void perform('activate')}>Attiva definitivamente</button></>}
        </div>
      </fieldset></form>

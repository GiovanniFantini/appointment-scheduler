...
export default function SubscriptionPlansPage() {
  ...
  return <div className="plans-page">
    <header className="plans-heading"><div><p className="plans-eyebrow">CATALOGO COMMERCIALE</p><h1>Pacchetti</h1><p>Funzioni e capacità per ogni azienda. Crea un pacchetto dedicato per le configurazioni personalizzate.</p></div>
      <button className="btn-primary" disabled={saving} onClick={() => { setDraft(emptyPlan()); setError(''); setSuccess('') }}>Nuovo pacchetto</button></header>
    {error && <div className="error-banner" role="alert">{error} <button onClick={() => { setError(''); void load() }}>Riprova</button></div>}
    ...
    {loading ? <p>Caricamento pacchetti…</p> : <div className="plans-catalog">
      {plans.map(plan => <article className="plan-card" key={plan.id}>
        ...
        <button className="btn-secondary" disabled={saving} onClick={() => { setDraft({ ...plan, features: [...plan.features] }); setError(''); setSuccess('') }}>Modifica pacchetto</button>
      </article>)}
      {plans.length === 0 && <p>Nessun pacchetto. Crea il primo per assegnarlo alle aziende.</p>}
    </div>}
    {draft && <form className="plan-editor" onSubmit={save}>
      <h2>{draft.id ? `Modifica: ${draft.name}` : 'Nuovo pacchetto'}</h2>
      <fieldset disabled={saving}>
        <div className="plan-fields"><label>Nome<input required maxLength={100} value={draft.name} onChange={e => setDraft({ ...draft, name: e.target.value })} /></label>
          <label>Descrizione<input maxLength={1000} value={draft.description ?? ''} onChange={e => setDraft({ ...draft, description: e.target.value })} /></label></div>
        <h3>Funzioni incluse</h3><div className="plan-feature-grid">{PLAN_FEATURES.map((feature, i) => <label key={feature}>
          <input type="checkbox" checked={draft.features.includes(i + 1)} onChange={e => setDraft({ ...draft, features: e.target.checked ? [...draft.features, i + 1] : draft.features.filter(f => f !== i + 1) })} />{feature}</label>)}</div>
        ...
        <div className="plan-fields">
          <label>Dipendenti attivi<input type="number" min={0} max={2147483647} step={1} placeholder="Illimitati" value={draft.maxEmployees ?? ''} onChange={e => setDraft({ ...draft, maxEmployees: e.target.value === '' ? null : Number(e.target.value) })} /></label>
          <label>Filiali attive<input type="number" min={1} max={2147483647} step={1} placeholder="Illimitate" value={draft.maxBranches ?? ''} onChange={e => setDraft({ ...draft, maxBranches: e.target.value === '' ? null : Number(e.target.value) })} /></label>
          <label>Spazio documenti (MiB)<input type="number" min={0} max={8589934591} step={1} placeholder="Illimitato" value={draft.maxStorageBytes == null ? '' : draft.maxStorageBytes / 1048576} onChange={e => setDraft({ ...draft, maxStorageBytes: e.target.value === '' ? null : Number(e.target.value) * 1048576 })} /></label>
        </div>
        ...
        <div className="plan-actions"><button className="btn-primary" type="submit">{saving ? 'Salvataggio…' : 'Salva pacchetto'}</button><button className="btn-secondary" type="button" onClick={() => setDraft(null)}>Annulla</button></div>
      </fieldset>
    </form>}
  </div>
}

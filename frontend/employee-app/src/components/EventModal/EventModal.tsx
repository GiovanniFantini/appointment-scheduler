...
export default function EventModal({ event, defaultDate, mode = 'full', onClose, onSaved }: EventModalProps) {
  ...
  const handleCreateExternal = async () => {
    ...
    setNewExtError('')
    try {
      const res = await apiClient.post('/employee/resources', {
  ...
  const handleSave = async () => {
    ...
    setLoading(true)
    try {
      const response = assignmentOnly
        ? await apiClient.put(`/events/${event!.id}/assignments`, buildAssignmentsPayload())
        : isEdit
  ...
  const removeSelectedParticipant = (employeeId: number) => {
    setSelectedOwnerIds(prev => prev.filter(id => id !== employeeId))
    ...
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal-container">
        ...
        <div className="modal-body">
                ...
                <>
                  ...
                  {selectedOwnerIds.length > 0 && (
                    <div className="selected-participants-panel">
                      <div className="selected-participants-title">Partecipanti selezionati</div>
                      <ul className="selected-participants-list">
                        {selectedOwnerIds.map(empId => {
                          const emp = employees.find(e => e.id === empId)
                          if (!emp) return null
                          const selectedSkillId = getParticipantSkillId(empId)
                          return (
                            <li key={empId} className="selected-participants-item">
                              <div className="selected-participants-name">
          ...
          {isEdit && !assignmentOnly && (eventType !== 'Turno' || step === 3) && (
            <>

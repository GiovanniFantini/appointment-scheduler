...
/** Client API per la timbratura operativa lato employee app. */
export const timeClockApi = {
  async getSettings(branchId: number): Promise<BranchTimeClockSettings> {
    const res = await apiClient.get<BranchTimeClockSettings>('/employee/time-clock-management/settings', {
      params: { branchId },
    })
    return res.data
  },
  ...
  async updateSettings(
    ...
    const res = await apiClient.put<BranchTimeClockSettings>(
      `/employee/time-clock-management/settings/${branchId}`,
      payload,
    )
    return res.data
  },
  ...
  async getEntries(params: {
    ...
    const res = await apiClient.get<TimeEntry[]>('/employee/time-clock-management/entries', { params })
    return res.data
  },

  async getAnomalies(params: { branchId?: number; status?: number }): Promise<TimeClockAnomaly[]> {
    const res = await apiClient.get<TimeClockAnomaly[]>('/employee/time-clock-management/anomalies', { params })
    return res.data
  },
  ...
  async approveAnomaly(id: number, reviewNotes?: string): Promise<TimeClockAnomaly> {
    const res = await apiClient.post<TimeClockAnomaly>(
      `/employee/time-clock-management/anomalies/${id}/approve`, { reviewNotes },
    )
    return res.data
  },
  ...
  async rejectAnomaly(id: number, reviewNotes?: string): Promise<TimeClockAnomaly> {
    const res = await apiClient.post<TimeClockAnomaly>(
      `/employee/time-clock-management/anomalies/${id}/reject`, { reviewNotes },
    )
    return res.data
  },
  ...
  async runDetection(branchId?: number): Promise<{ created: number }> {
    const res = await apiClient.post<{ created: number }>(
      '/employee/time-clock-management/run-detection', null, { params: { branchId } },
    )
    return res.data
  },
  ...
  async getReport(params: {
    ...
    const res = await apiClient.get<TimeClockReportRow[]>('/employee/time-clock-management/report', { params })
    return res.data
  },
}
